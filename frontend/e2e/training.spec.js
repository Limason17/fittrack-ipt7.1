...
test('Workout CRUD, kg/lb, Progress-Herkunft, Unveränderlichkeit und 1RM', async ({ page, request }) => {
  ...
  const createWorkout = page.getByRole('button', { name: 'Workout erstellen' })
  await createWorkout.focus()
  ...
  await exerciseTrigger.focus()
  await page.keyboard.press('Enter')
  await chooseExercise(page, 'Bench Press', { keyboard: true })
  ...
  const saveWorkout = workoutForm.getByRole('button', { name: 'Workout speichern' })
  await saveWorkout.focus()
  await page.keyboard.press('Enter')
