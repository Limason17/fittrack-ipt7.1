...
**Korrektur Stage 5D (2026-08-19, `docs/STAGE_5D_CURRENT_STATE_AUDIT.md`)**: zwei vollständige, seit Stage 5A1/5A3 gemountete Router-Module fehlten in diesem Katalog komplett — Abschnitte „Training Calendar" (`trainingCalendarV1.js`) und „Assignment Schedule Rules" (`assignmentScheduleRuleV1.js`) unten neu ergänzt, direkt gegen den Router-Quellcode verifiziert. Ausserdem war die Spalte „Frontend" für die gesamte Workout-Session-/Coach-Resultat-Gruppe seit Stage 1B.2B2A/1B.2B2B veraltet (behauptete durchgängig „Kein Frontend-Nutzer") — korrigiert.

**Merge-Gate-Nachtrag Stage 5D (2026-08-19)**: ein anschliessender, vollständig deterministischer Route-vs-Katalog-Abgleich (jede Router-Factory direkt geladen, echter `Router.stack` ausgelesen, kein Text-/Regex-Zählen) deckte zwei zuvor unbemerkte, echte Lücken auf — der komplette Abschnitt „Auth Session" (`authSessionRouter.js`, Stage 3B2: `refresh`/`logout`/`logout-all`) fehlte vollständig, und in „Invitations" fehlte die `resend`-Zeile (Stage 3C). Beide jetzt ergänzt. Damit ist dieser Katalog **erstmals vollständig 1:1 gegen den realen, gemounteten Route-Bestand verifiziert**: 92 tatsächliche Endpunkte, 92 katalogisierte Zeilen, keine offene Differenz — siehe `docs/STAGE_5D_CURRENT_STATE_AUDIT.md` Abschnitt 16/16.1 für die vollständige Belegtabelle und Methodik.
...
| **P1 – Konto/Identität** | Account-Metadaten, Einstellungen | Auth/Users, Auth Session, Account |
...
## Auth Session — `backend/routes/authSessionRouter.js`, mounted at `/api/auth` (Stage 3B2 Session Hardening — bislang nicht in diesem Katalog erfasst, Stage-5D-Ergänzung)

Cookie-/CSRF-basierte Sitzungsverwaltung, ergänzend zum reinen Bearer-JWT der übrigen Router: `/refresh` liest den HttpOnly-Refresh-Cookie plus den `X-CSRF-Token`-Header (kein `Authorization`-Header, kein Request-Body), rotiert den Refresh-Token einmalig verwendbar und liefert einen neuen Access-Token; `/logout`/`/logout-all` erfordern zusätzlich `Authorization: Bearer`. Alle drei Routen erzwingen `verifyOrigin`+`verifyCsrf` und setzen `Cache-Control: no-store`. Frontend-seitig kein eigener View-Aufrufer — `/refresh` läuft app-weit im stillen Session-Bootstrap (`utils/api.js`), `/logout`/`/logout-all` werden aus `utils/auth.js` heraus von `ProfileView.vue`s Sicherheits-Tab ausgelöst.

| Methode | Pfad | Auth | Rollen | Tenant | Zweck | Wichtige Request-Felder | Response (Top-Level) | Pagination | Fehlercodes | Datenschutz | Frontend | Tests |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| POST | `/api/auth/refresh` | Refresh-Cookie + CSRF-Header (30/5min/IP) | n/a | n/a | Access-Token erneuern, Refresh-Token rotieren | — (Cookie `fittrack_refresh` + Header `X-CSRF-Token`) | accessToken | Nein | 401 `AUTH_REFRESH_TOKEN_INVALID`/`_EXPIRED`/`_REUSE_DETECTED`, 403 `AUTH_CSRF_INVALID`/`AUTH_ORIGIN_NOT_ALLOWED`, 429 | P1 | `utils/api.js` (app-weiter Silent-Refresh, kein einzelner View) | Unit+Integration+E2E |
| POST | `/api/auth/logout` | JWT + CSRF-Header | any | n/a | Aktuelle Sitzung widerrufen (idempotent) | — | message | Nein | 401, 403 `AUTH_CSRF_INVALID`/`AUTH_ORIGIN_NOT_ALLOWED` | P1 | `utils/auth.js` → ProfileView.vue | Integration+E2E |
| POST | `/api/auth/logout-all` | JWT + CSRF-Header (10/15min/User) | any | n/a | Alle Sitzungen des Kontos widerrufen | — | message | Nein | 401, 403 `AUTH_CSRF_INVALID`/`AUTH_ORIGIN_NOT_ALLOWED`, 429 | P1 | `utils/auth.js` → ProfileView.vue | Integration+E2E |
...
| POST | `.../invitations/:invitationId/resend` | JWT (5/15min/Akteur+Einladung) | owner/admin (`INVITATION_RESEND`, kein Trainer-Zugriff) | Studio-Kontext | Einladung erneuern (Tokenrotation, In-Place bei bereits abgelaufener Einladung) | — | invitation{...}, delivery{...} | Nein | 401, 403, 404, 409 `INVITATION_ALREADY_ACCEPTED`/`INVITATION_REVOKED`/`INVITATION_NOT_RESENDABLE`/`INVITATION_EMAIL_ALREADY_MEMBER`/`INVITATION_RESEND_CONFLICT`, 429 `INVITATION_RESEND_RATE_LIMITED`, 502 `INVITATION_DELIVERY_FAILED`, 503 `INVITATION_DELIVERY_RECOVERY_FAILED` | P3 (E-Mail = PII) | StudioInvitationsView.vue | Component+Integration+E2E |
...
**[KORRIGIERT, Stage 5D, 2026-08-19 — vollständig deterministisch aufgelöst]** Beide Aussagen waren zum Stand PR #7 (2026-07-19) zutreffend und sind es seit Stage 1B.2B2A/B, 3B2, 3C, 5A1 und 5A3 nicht mehr:

- **Dieser Katalog dokumentiert nach der Stage-5D-Merge-Gate-Prüfung exakt 92 Endpunkte** über **11 Router-Module** (`users`, `authSessionRouter`, `accountRouter`, `exercises`, `workouts`, `progress`, `trainingCalendarV1`, `studioV1`, `trainingProgramV1`, `assignmentScheduleRuleV1`, `workoutSessionV1`) plus 3 Inline-Health-Routen — **92 tatsächlich gemountete Produkt-Endpunkte, 92 katalogisierte Zeilen, Differenz 0.**
- Ermittelt durch ein deterministisches Skript, das jede Router-Factory direkt lädt und deren echten Express-`Router.stack` ausliest (nicht Text-/Regex-Zählung), jede Route mit ihrem tatsächlichen `app.use(...)`-Mount-Präfix aus `startup/app.js` zum effektiven Pfad zusammensetzt und auf Duplikate prüft: **92 rohe Route-Registrierungen = 92 eindeutige (Methode+Pfad)-Kombinationen, keine Duplikate, keine toten/nicht gemounteten Router.** Jede der 92 Routen wurde anschliessend genau einer Kategorie zugeordnet (A korrekt dokumentiert / B fehlte im Katalog / C bewusst nicht separat katalogisiert / D Alias-/Doppelregistrierung / E tot) — Kategorien C, D und E blieben **leer**.
- Die frühere Differenz **92 vs. 88 war kein Zählfehler**, sondern exakt vier real fehlende, produktiv gemountete Endpunkte (Kategorie B): das komplette `authSessionRouter.js` (`POST /api/auth/refresh`, `/logout`, `/logout-all`, Stage 3B2 — bislang **ohne eigenen Abschnitt** in diesem Katalog) sowie `POST .../invitations/:invitationId/resend` (Stage 3C — Abschnitt "Invitations" existierte, die Zeile fehlte). Alle vier jetzt ergänzt (siehe "Auth Session" oben und die neue Zeile in "Invitations"), direkt aus Router-/Service-Quellcode abgeleitet, keine erfundenen Verträge.
- Die Workout-Session-Gruppe (10 Endpunkte, weiterhin Datenschutzklasse P4) hat **seit Stage 1B.2B2A** (Member-Ausführungs-UI, PR #9) und **Stage 1B.2B2B** (Coach-Resultat-/Feedback-UI, PR #10) reale Frontend-Aufrufer — `WorkoutSessionView.vue`, `MyTrainingPlanView.vue`, `WorkoutSessionHistoryView.vue`, `CoachResultsView.vue`, `CoachSessionDetailView.vue`.
- Zuvor bereits in diesem Audit ergänzt: die komplette Training-Calendar-Gruppe (Stage 5A1/5A2, 7 Endpunkte) und die Assignment-Schedule-Rules-Gruppe (Stage 5A3, 3 Endpunkte).

Volle Methodik und Belegtabelle: `docs/STAGE_5D_CURRENT_STATE_AUDIT.md` Abschnitt 16/16.1.
