...
**Nachträgliche Re-Verifikation (Merge-Readiness-Prüfung):** Der Commit-Diff wurde erneut Zeile für Zeile gelesen. Die Implementierung ist korrekt: `today` wird über `todayInTimezone(timezoneRow.default_timezone || DEFAULT_PERSONAL_TIMEZONE)` berechnet, `default_timezone` stammt aus einer expliziten `SELECT default_timezone FROM studios WHERE id = ?`-Abfrage für genau dieses Studio, und `CURDATE()` kommt im aktuellen Code an dieser Stelle nicht mehr vor (nur noch als erklärender Kommentar über den historischen Fehler). Der Commit-**Titel** „Fix workout session calendar link using the DB server's timezone" ist jedoch missverständlich formuliert — er liest sich, als würde der Fix selbst die DB-Serverzeitzone verwenden, während tatsächlich das Gegenteil der Fall ist (die DB-Serverzeitzone war der Fehler, die Studio-Zeitzone ist der Fix; der Commit-Body beschreibt das korrekt). Da die Implementierung korrekt ist, wurde **keine Historie umgeschrieben und kein Force-Push durchgeführt** — dies wird hier lediglich als Klarstellung dokumentiert. Zusätzlich wurde ein neuer, deterministischer Backend-Integrationstest ergänzt (`test/integration/trainingCalendarApi.test.js`, „a studio whose local day differs from the database server's UTC day still links the correct calendar occurrence"): er wählt dynamisch eine von zwei festen Extrem-Zeitzonen (`Pacific/Kiritimati`, UTC+14, und `Etc/GMT+12`, UTC−12 — zusammen decken sie jede volle UTC-Stunde ab), je nachdem welche der beiden gerade tatsächlich vom UTC-Kalendertag abweicht, sodass der Test unabhängig von der Tageszeit, zu der die Suite läuft, immer eine echte Divergenz prüft. Verifiziert: schlägt gegen den Vor-Fix-Code reproduzierbar fehl (denselben Fehlerzustand wie oben beschrieben), besteht gegen den aktuellen Code.
...
## 19. Merge-Readiness-Nachbereitung: drei zusätzliche Probleme gefunden und behoben

Ein erster Merge-Versuch wurde zurückgestellt, weil der vollständige Chromium-E2E-Lauf nicht durchgängig grün war. Die folgenden drei Ursachen wurden einzeln reproduziert, per Trace-/Netzwerk-/Codeanalyse auf ihre tatsächliche Ursache zurückgeführt und minimal behoben — keine pauschalen Timeouts, keine zusätzlichen Retries, kein `test.skip`, keine abgeschwächten Assertions, kein `serial` entfernt.

### 19.1 `calendar.spec.js` „Persönlicher Ablauf" (Reschedule-Flow) — jetzt behoben, nicht mehr nur dokumentiert

Der Test schlug bei ca. 40 % der Läufe fehl: `findEvent()` fand nach dem Verschieben ein Element, der anschliessende `.click()` hing danach bis zum Timeout mit „element was detached from the DOM, retrying". Per extrahiertem Playwright-Trace (Netzwerk- und Aktions-Log) wurde die tatsächliche Ursache zweifelsfrei belegt: `CalendarView.vue#handleRescheduleSubmit` zeigt den Erfolgs-Toast synchron an und lädt den sichtbaren Kalenderbereich erst danach per `await loadRange()` neu — ein normales, korrektes „optimistischer Toast, Hintergrund-Resync"-Muster, **kein** Produktfehler. Das im Test gewählte Verschiebedatum (Ausgangsdatum + 2 Tage) verlässt bei diesem Testlauf absichtlich den aktuell geladenen Monatsbereich (der Test verschiebt über eine Monatsgrenze hinweg), sodass der korrekte Endzustand ist: das Element verschwindet vollständig aus der aktuellen Ansicht. Der Test rannte jedoch sofort nach dem sichtbaren Toast weiter und konnte dabei das noch nicht aktualisierte, gleich verschwindende DOM-Element abgreifen — der nachfolgende `.click()` verlor es dann mitten in der Aktion, ohne dass ein regulärer `.click()` (anders als `findEvent()`s eigene Monatswechsel-Retry-Logik) je einen Wechsel zum nächsten Monat versucht hätte. Behoben durch Warten auf die tatsächliche, durch die Reschedule-Mutation ausgelöste `GET /api/v1/training-calendar`-Netzwerkantwort (`page.waitForResponse(...)`), bevor der Test überhaupt wieder nach dem Element sucht — ein echtes, ereignisbasiertes Synchronisationssignal, kein `waitForTimeout`. Verifiziert: 5× einzeln grün, 3× die vollständige `calendar.spec.js`-Datei grün (insgesamt 8 konsekutive fehlerfreie Läufe nach dem Fix, gegenüber vorher 2 von 5 Fehlschlägen).

### 19.2 Geteiltes E2E-Registrierungs-Rate-Limit durch Stage 5A3s eigene neue Tests überschritten

Nach dem obigen Fix zeigte ein vollständiger Suite-Lauf einen **neuen, anderen** Fehlschlag: `workoutSessions.spec.js` erhielt beim Registrieren eines Testnutzers `429 RATE_LIMIT_EXCEEDED`. Ursache: `AUTH_REGISTRATION_RATE_LIMIT_MAX` war in `playwright.config.js` auf `100` gesetzt (IP-geschlüsselt, 60-Minuten-Fenster), während die gesamte Suite (ein gemeinsames Backend, eine gemeinsame Loopback-IP, ein einziges 60-Minuten-Fenster für den kompletten ca. 5-6-minütigen Lauf) über alle 59 Tests hinweg deutlich mehr Registrierungen als 100 erzeugt — Stage 5A3s eigene neue Tests (`coachScheduling.spec.js`: 6 zusätzliche Registrierungen; der neue Axe-Test: 2 weitere) haben diesen bereits eng bemessenen, gemeinsamen Zähler über die Grenze geschoben. Dieselbe Datei enthält für exakt dasselbe Muster bereits einen dokumentierten, etablierten Präzedenzfall: `AUTH_REFRESH_RATE_LIMIT_MAX`, `AUTH_LOGOUT_ALL_RATE_LIMIT_MAX`, `INVITATION_CREATE_RATE_LIMIT_MAX` und `INVITATION_ACCEPT_RATE_LIMIT_MAX` wurden bereits früher aus genau demselben Grund („diese Suite erzeugt legitimen, wachsenden Testverkehr, keinen Missbrauch") grosszügig auf `1000` gesetzt — nur die Registrierung war davon bisher ausgenommen geblieben. Behoben durch dieselbe Anpassung (`AUTH_REGISTRATION_RATE_LIMIT_MAX: '1000'`), mit derselben Begründung dokumentiert; **reine E2E-Testkonfiguration**, der reale Produktions-Default (5/60 Min, `rateLimitPolicies.js`) ist unverändert. Kein bestehender Test prüft einen konkreten Wert dieses Limits, daher keine Kollision mit einer bestehenden Assertion.

### 19.3 Axe-Toast-Übergangs-Race in den Kalender-/Zeitplan-Accessibility-Tests

Der neue Stage-5A3-Axe-Test schlug bei einem weiteren vollständigen Lauf mit einem `color-contrast`-Fund (Kontrastverhältnis 2.37, Schwelle 4.5) auf dem Erfolgs-Toast fehl. Rechnerisch nachgeprüft: der tatsächliche, eingeschwungene Kontrast von weissem Text auf `--accent-hover` (`#155a3b`) beträgt **8.21:1** — deutlich über der Anforderung. Die von Axe gemessenen Farben (`#c8d8d1` auf `#66927f`) entsprechen keiner im CSS definierten Farbe; sie sind das Ergebnis eines mitten in der 180ms-Eintrittsanimation (`ToastHost.vue`, `transition: opacity`) ausgeführten Scans, bei dem Axe die tatsächlich zu diesem Zeitpunkt zusammengesetzte, teiltransparente Pixel-Farbe misst statt der späteren, voll sichtbaren. **Kein Design-/Kontrastfehler, sondern erneut eine Testsynchronisierungslücke** derselben Art wie 19.1. Behoben durch Warten auf das Verschwinden der von Vue selbst verwalteten `toast-enter-active`-Klasse (`expect(page.locator('.toast-enter-active')).toHaveCount(0)`) — ein echtes, DOM-Zustands-basiertes Signal für „Übergang tatsächlich abgeschlossen", keine geschätzte Wartezeit. Angewendet an der betroffenen Stelle im neuen Stage-5A3-Test **und** vorsorglich an der strukturell identischen, bisher nur zufällig nicht ausgelösten Stelle im bestehenden Stage-5A2-Kalender-Axe-Test. Verifiziert: 3× beide Tests zusammen grün.

Alle drei Fixes wurden anschliessend gemeinsam durch zwei vollständige, saubere Chromium-Gesamtläufe bestätigt (Abschnitt 21).
...
**Backend-Integration** (zusätzlich zur bestehenden Suite, jetzt 254 Tests): ein neuer, deterministischer Test für die Zeitzonen-Divergenz (Abschnitt 1) in `trainingCalendarApi.test.js`.
...
## 21. Regression (final, nach Merge-Readiness-Nachbereitung)

Backend: `npm test` (Unit 508/508, Integration 254/254 inkl. der MinIO-abhängigen Remote-Backup-Suite nach Start des lokalen MinIO-Testcontainers und des neuen Zeitzonen-Divergenz-Tests, Migrations 32/32, Syntax 230 Dateien), `npm run audit:security` (0 Schwachstellen), Migration Doctor `ready`. Frontend: `npx vitest run` (499/499), `npm run build` (erfolgreich), `npm audit` (0 Schwachstellen). E2E: **zwei vollständige, unabhängige Chromium-Gesamtläufe, beide 59/59 bestanden, 0 fehlgeschlagen, 0 übersprungen, keine Retries** — der zuvor beschriebene Reschedule-Flake (Abschnitt 19.1), das Registrierungs-Rate-Limit (19.2) und die Axe-Toast-Race (19.3) sind damit nicht mehr nur dokumentiert, sondern nachweislich behoben.
