...
> ## ⚠️ Aktueller Stand (2026-08-19, nach Stage 5C2): siehe `STAGE_5D_CURRENT_STATE_AUDIT.md`
>
> **Die Abschnitte 1–13 sowie der Anhang dieses Dokuments sind ein bewusst
> eingefrorener Snapshot vom 2026-07-19 (PR #7, Stage 1B.2B1)** — siehe die
> Nachtrag-Kette direkt darunter, die diesen Stand seither Phase für Phase
> fortschreibt. Weder der Snapshot-Körper noch eine einzelne Nachtrag-Zeile
> für sich genommen ist die aktuelle Quelle: **`docs/STAGE_5D_CURRENT_STATE_AUDIT.md`
> ist die aktuelle, vollständige und gegen den heutigen Code/die heutigen
> Tests neu gemessene Bestandsaufnahme** (Funktionsinventar, Migrationen,
> Testzahlen, Security-/Privacy-Status, Pilot-/Produktions-Readiness). Zwei
> Aussagen weiter unten sind besonders auffällig veraltet und werden hier
> ausdrücklich richtiggestellt, damit sie nicht isoliert als aktueller
> Zustand missverstanden werden: Abschnitt 8 behauptet "Es existiert kein
> Recht-auf-Löschung-/Anonymisierungsmechanismus" — das ist seit Stage 5C1
> (Backend) und Stage 5C2 (Profil-Oberfläche) **nicht mehr zutreffend**; die
> in Abschnitt 1/11 beschriebene "keine Workout-Session-UI"-Lücke ist bereits
> seit Stage 1B.2B2A (PR #9) geschlossen, wie die erste Nachtrag-Zeile direkt
> unten selbst schon festhält.
>
> ---
...
>
> **Nachtrag (2026-07-26, Stage 4A Final Local Acceptance):** Eine vollständige lokale Abnahme wurde durchgeführt: Clean-Room-Installation (isolierter Git-Worktree, ohne `node_modules`/`.env`/Datenbank) nach `README.md`, frische und bestehende Datenbank beide `ready` ohne Drift, vollständige Rollen-/Tenant-/Auth-/Session-/Einladungs-/Programm-/Workout-/Account-/Rate-Limit-/CORS-Regression (469 Backend-Unit-, 231 Backend-Integrations-, 32 Migrationstests, 341 Frontend-Tests, dreimal 48/48 grüne Chromium-E2E-Läufe, 20-facher Cross-Tab-Zieltest grün, alle ohne Flake), ein realer verschlüsselter Backup-/Restore-Drill gegen einen realistischen Mehrrollen-/Mehrstudio-Datenbestand sowie ein synthetischer Production-Config-Smoke-Test. Ein während der wiederholten Regression reproduzierter, aber bereits am Ende von Stage 3D behobener Lock-Order-Deadlock (`rotateRefreshToken` vs. Passwort-/E-Mail-Änderung) blieb stabil behoben. `README.md` wurde als veraltet identifiziert (fehlende Studios/Rollen/Coaching/Rate-Limiting/E2E-Erwähnung seit dem frühen Stage-1B.2B1-Stand) und überarbeitet; ein neues `docs/LOCAL_PILOT_RUNBOOK.md` beschreibt einen vollständigen, tatsächlich ausgeführten lokalen Pilotablauf. Vollständiger Bericht in `STAGE_4A_FINAL_LOCAL_ACCEPTANCE.md`. **FitTrack gilt damit als lokal vollständig abgeschlossen (local product development complete) — keine Produktions-Deployment-Behauptung, keine weitere lokale Entwicklungsphase folgt.** Stage 2B2B bleibt weiterhin **Deferred until first customer / production deployment**; es wurde auch für Stage 4A keine Cloud-Infrastruktur eingerichtet.
>
> **Nachtrag (2026-07-26, Stage 5A1 Unified Training Calendar — Backend Foundation):** Nach dem in Stage 4A erklärten "local product development complete" wurde diese neue, separat beauftragte Phase begonnen: das Backend-Fundament für einen vereinheitlichten persönlichen Trainingskalender (persönliche manuelle Trainings, persönliche abgeschlossene Workouts, coach-geplante Studio-Workouts, gestartete/abgeschlossene/übersprungene/abgesagte Trainings in einer Ansicht). Migration 012 (`012_unified_training_calendar`) fügt `studio_assignment_schedule_rules` (wiederkehrende Coach-Terminierungsregeln) und `training_calendar_entries` (Vorkommnisse/Status) hinzu sowie `public_id` auf der alten `workouts`-Tabelle. Architektur: "Scheduling Rules + lazy, idempotent materialisierte Occurrences" — künftige Kalendertage werden nie im Voraus für die gesamte Zukunft erzeugt, sondern beim Lesen bedarfsgerecht und begrenzt auf max. 93 Tage materialisiert; einmal materialisierte Zeilen sind gegenüber späteren Regeländerungen unveränderlich. Neue Lese-API (`GET /api/v1/training-calendar`) und Mutations-APIs für persönliche Einträge, plus Terminierungsregel-Verwaltung für Owner/Admin/Trainer (rollenbasiert, tenant-isoliert). Verknüpfung mit der bestehenden Workout-Session-Ausführung (Start/Abschluss/Abbruch kaskadieren atomar in den Kalenderstatus). Keine Datenduplizierung — bestehende `workouts`/`progress_entries` bleiben unverändert, historische persönliche Workouts erscheinen über ein vereinigtes Lesemodell ohne Migration. **Es gibt in dieser Phase keine Kalender-Oberfläche** (Stage 5A2, bewusst nicht begonnen). Während der vollständigen Regressionssuite wurden zwei durch die neue Migration ausgelöste Regressionen in bestehendem Code gefunden und behoben: ein fehlendes `public_id` im bestehenden `POST /workouts`-Insert-Pfad, sowie vier bestehende Migrationstests mit fest auf `11` codierter erwarteter Migrationsanzahl/-liste (jetzt `12`). Vollständiger Bericht in `STAGE_5A1_UNIFIED_CALENDAR_BACKEND.md`. Migration 012 angewendet (`applied:12`, Doctor `ready` mit 0 Problemen, Restore-Drill gegen die reale Entwicklungsdatenbank erfolgreich). Stage 2B2B bleibt weiterhin **Deferred until first customer / production deployment**; es wurde für Stage 5A1 keine Cloud-Infrastruktur eingerichtet.
>
> **Nachtrag (2026-07-26, Stage 5A2 Personal Calendar UI):** Die in Stage 5A1 explizit offengelassene Kalender-Oberfläche ist jetzt geliefert: eine persönliche Kalenderseite (`/calendar`, neuer Sidebar-Eintrag) mit Desktop-Monatsraster und mobiler Agenda-Ansicht, die persönliche und Studio-Trainings in einer Ansicht zusammenführt. Statusfarben (PLANNED blau, DUE_TODAY violett, OVERDUE orange, IN_PROGRESS indigo, COMPLETED grün, SKIPPED/CANCELLED rot mit unterschiedlichem Text/Icon), Event-Detaildialog mit ausschliesslich serverseitig erlaubten Aktionen (`availableActions`), persönliche Trainingserstellung mit den Zukunft-/Heute-/Vergangenheit-Regeln aus Stage 5A1, Bearbeiten/Verschieben, Bestätigen/Überspringen/Absagen mit Bestätigungsdialogen, Studio-Workout-Start über den bestehenden Session-Vertrag, 409-Konfliktbehandlung mit automatischem Neuladen, clientseitige Quell-/Statusfilter, vollständige DE/EN-Übersetzung. **Es gibt weiterhin keine Coach-Planungsoberfläche** (Stage 5A3, bewusst nicht begonnen) und **keine Migration 013** — reine Frontend-Phase. Während der Verifikation des tatsächlichen Stage-5A1-API-Vertrags wurden zwei echte, reproduzierbare Backend-Lücken gefunden und mit minimalinvasivem Fix behoben: `revision` und die Programmzuweisungs-`assignmentId` fehlten in jeder Kalenderantwort, obwohl beide für die geforderten Mutations- bzw. Start-Aktionen zwingend nötig sind (rein additive Response-Felder, keine Migration, 4 neue Backend-Integrationstests). Ein echter browserbasierter manueller Testlauf deckte zusätzlich zwei UI-Bugs auf und behob sie: ein leerer Kalendermonat entfernte zuvor die gesamte Navigation, und der Statustext war in der kompakten Rasteransicht nur für Screenreader, nicht sichtbar vorhanden. Vollständiger Bericht in `STAGE_5A2_PERSONAL_CALENDAR_UI.md`. Stage 2B2B bleibt weiterhin **Deferred until first customer / production deployment**; es wurde für Stage 5A2 keine Cloud-Infrastruktur eingerichtet.
>
> **Nachtrag (2026-07-27, Stage 5A3 Coach Scheduling UI):** Die in Stage 5A2 explizit offengelassene Coach-Planungsoberfläche ist jetzt geliefert: unter einer neuen Route je Programmzuweisung (`/studios/:studioId/program-assignments/:assignmentId/schedule`, erreichbar über eine neue „Zeitplan"-Aktion in der Zuweisungsliste) verwalten Owner/Admin/eigene Trainer Terminierungsregeln — welcher Programmtag an welchem Wochentag, ab wann, mit welcher Wiederholung (wöchentlich bis alle 52 Wochen) und optional bis wann stattfindet. Regelübersicht mit lesbarer Wiederholungszusammenfassung statt roher Felder, Erstellen/Bearbeiten (nur die tatsächlich vom Backend unterstützten Felder, nie der Programmtag nachträglich), Deaktivieren (kein Hard-Delete, bereits abgeschlossene Trainings bleiben nachweislich unangetastet), optionale, aus der bereits getesteten Backend-Domänenlogik zeilengetreu portierte Terminvorschau. **Es gibt weiterhin keine Migration 013** — nahezu reine Frontend-Phase. Während der Erstellung des Pflicht-E2E-Szenarios (Member startet und schliesst ein automatisch generiertes Studio-Training ab) wurde ein echter, von Stage 5A3 unabhängiger Blocker gefunden und nach dem vorgeschriebenen Sechs-Schritte-Prozess minimal behoben: `workoutSessionService.js#startSession()` bestimmte "heute" für die Kalenderverknüpfung über die Zeitzone des Datenbankservers (`CURDATE()`, hier `UTC`) statt der Studio-Zeitzone (`Europe/Zurich`), wodurch die Verknüpfung in einem täglichen Zeitfenster um Mitternacht (CEST: 00:00–02:00 lokal) fehlschlug — verifiziert auf einem sauberen `main`-Checkout als bereits vorher vorhanden, nicht durch diese Phase verursacht. Ein echter browserbasierter manueller Testlauf deckte zusätzlich einen UI-Bug auf und behob ihn: die „Zeitraum"-Spalte zeigte für Regeln ohne Enddatum den wörtlichen Dropdown-Platzhaltertext statt eines Gedankenstrichs.
>
> **Nachtrag (2026-07-27, Stage 5A3 Merge-Readiness-Nachbereitung):** Ein bereits vor Stage 5A3 bestehender, unabhängiger Flaky-Test im persönlichen Kalender-Verschiebe-Ablauf (zunächst nur dokumentiert, siehe oben) wurde bei der Merge-Vorbereitung tatsächlich behoben: eine Testsynchronisierungslücke (Race zwischen dem sofort sichtbaren Erfolgs-Toast und dem erst danach abgeschlossenen Kalender-Refetch), kein Produktfehler. Zwei weitere, davon unabhängige Probleme wurden im selben Zug gefunden und behoben: ein durch Stage 5A3s eigene neue E2E-Tests überschrittenes, geteiltes Registrierungs-Rate-Limit in der E2E-Testkonfiguration (reine Testkonfiguration, kein Produktions-Verhalten geändert), sowie ein Axe-Farbkontrast-Fund auf dem Erfolgs-Toast, der sich als Scan-mitten-in-der-Eintrittsanimation herausstellte (eingeschwungener Kontrast 8.21:1, weit über der Anforderung — kein Design-Fehler). Der zuvor als Backend-Blocker dokumentierte Zeitzonen-Fix wurde erneut am Code verifiziert (korrekt) und um einen deterministischen Integrationstest ergänzt. Zwei vollständige, unabhängige Chromium-Gesamtläufe danach beide 59/59 grün, 0 fehlgeschlagen, 0 übersprungen. Details in `STAGE_5A3_COACH_SCHEDULING_UI.md` Abschnitt 19. Vollständiger Bericht in `STAGE_5A3_COACH_SCHEDULING_UI.md`. Stage 2B2B bleibt weiterhin **Deferred until first customer / production deployment**; es wurde für Stage 5A3 keine Cloud-Infrastruktur eingerichtet.
>
> **Nachtrag (2026-07-27, Stage 5B Product & Pilot Readiness Audit):** Nach dem Merge von Stage 5A3 (PR #23) wurde ein vollständiges, rein evidenzbasiertes Audit auf einem eigenen Branch (`audit/stage-5b-product-pilot-readiness`, kein Merge nach `main`) durchgeführt — siehe `STAGE_5B_PRODUCT_PILOT_READINESS_AUDIT.md`. Ergebnis: **0 P0-Blocker**, Gesamtklassifikation **„Ja, mit einer Bedingung“** — pilotbereit, sofern vor Aufnahme echter, nicht-anonymer Teilnehmer:innen ein — und sei es rein manueller — Löschprozess für Personendaten definiert wird (bislang existiert im Schema keinerlei Lösch-/Anonymisierungsmechanismus, unverändert seit Stage 3A). Vollständig real reproduziert und **alle grün**: Backend-Regression (508 Unit-, 254 Integrations- inkl. Off-host-Backup-, 32 Migrationstests, 0 Schwachstellen ≥ high), Frontend-Regression (499 Tests, 0 Schwachstellen, erfolgreicher Produktionsbuild), Migration Doctor (`ready:true, applied:12`), volle Chromium-E2E-/Axe-Suite (59/59, 0 fehlgeschlagen) sowie ein zusätzliches, eigens für dieses Audit real durchgeführtes 18-Schritte-Cross-Role-Szenario über alle vier Rollen inklusive doppelt bestätigter Verlaufskonsistenz nach nachträglicher Terminierungsregel-Änderung. Wichtigster inhaltlicher Neufund: Die Studio-Übersicht (`StudioDashboardView.vue`) liefert für Owner/Admin/Trainer **keine** operative Tagesübersicht (keine überfälligen Trainings, keine neuen Ergebnisse, keine inaktiven Mitglieder) — durch zwei Screenshots vor und nach vollständiger Studio-Nutzung als inhaltlich identisch bestätigt, obwohl alle dafür nötigen Rohdaten bereits existieren. Zusätzlich gefunden: neuere Kalender-/Terminierungs-Audit-Ereignistypen erscheinen im Audit-Log nur als unübersetzter Fallback (die bestehende Übersetzungs-Regressionsprüfung deckt diese beiden neuen Präfixe noch nicht ab). Zwei in `FITTRACK_SECURITY_AND_PRIVACY_STATUS.md` weiterhin gelistete „offene“ Punkte (CORS, Rate-Limiting) sind tatsächlich seit Stage 3D gelöst — die Dokumentliste war schlicht nicht mit ihrem eigenen Nachtrag synchronisiert (jetzt in diesem Audit korrigiert). Empfehlung für die nächste Phase: **kein** weiteres Kalender-Feature, sondern ein schlankes „Stage 5C — Studio Operations Dashboard“, das genau die gefundene operative Lücke schliesst (Details in Abschnitt 32 des Audit-Dokuments). Stage 2B2B bleibt weiterhin **Deferred until first customer / production deployment**; es wurde für Stage 5B keine Cloud-Infrastruktur eingerichtet und keine neue Entwicklungsphase begonnen.
>
> **Nachtrag (2026-07-28, Stage 5C1 Account Deletion Backend & Deletion Receipt Foundation):** Der in Stage 5B als einzige Pilot-Bedingung benannte Punkt — "bislang existiert im Schema keinerlei Lösch-/Anonymisierungsmechanismus für Personendaten" — ist jetzt geschlossen, ausschliesslich backendseitig. Auf einem eigenen, separat beauftragten Design-Vorlauf (`docs/STAGE_5C_PERSONAL_DATA_LIFECYCLE_DESIGN.md`, ADR 004) aufbauend liefert Migration 013 (`013_account_lifecycle`, additiv: `lifecycle_status`/`deleted_at` auf `users`) und ein vollständiger Self-Service-Löschfluss: Vorschau (`GET /api/account/deletion-preview`) und Ausführung (`POST /api/account/deletion-request`, Passwort + Bestätigungsphrase = eigener Benutzername) über eine einzige, in beiden Pfaden identisch genutzte Planungsfunktion, eine 17-Schritte-atomare Transaktion (Sessions/Zuweisungen/Coaching-Beziehungen/Terminierungsregeln/Kalendereinträge terminalisieren, persönliche Daten hart löschen, alle Mitgliedschaften auf `left`, alle Sessions/Refresh-Tokens widerrufen), ein Sole-Owner-Blocker (kein Konto kann sich löschen, solange es in einem Studio der einzige aktive Owner ist), sowie Hybrid-Löschstrategie: Anonymisierung (kryptographisch zufälliger, nie ableitbarer Platzhaltername/-E-Mail/-Passworthash) für Konten mit Studio-Historie, vollständiger Hard Delete für Konten ohne jede je existierende Studio-Mitgliedschaft. Extern signierte, atomar veröffentlichte Deletion Receipts (HMAC-SHA256, ausserhalb des Repositories) belegen jede Löschung unabhängig von der Datenbank selbst; ein eigener Deletion Receipt Doctor und ein dreifach-acknowledgement-gated Restore-Reconciliation-Mechanismus behandeln den Fall, dass ein Backup-Restore ein bereits gelöschtes Konto versehentlich reaktiviert. **Es gibt in dieser Phase weiterhin keine Frontend-Oberfläche** (weder Profil-Danger-Zone/Bestätigungsdialog noch Studio-Membership-Removal-UI — beides bewusst Stage 5C2, nicht begonnen); die Löschung ist aktuell ausschliesslich über die API erreichbar. Während der vollständigen Regressionssuite wurden drei durch die neue Spalte/den neuen Pflicht-Rate-Limiter ausgelöste Regressionen in bestehendem Code gefunden und behoben (ein bestehendes Integrationstest-Router-Fixture ohne den neuen Limiter, eine weitere fest auf `12` codierte Migrationsanzahl-Assertion, eine bestehende Auth-Middleware-Test-Fixture ohne die neue Spalte) — Details in `STAGE_5C1_ACCOUNT_DELETION_BACKEND.md` Abschnitt 19. Vollständig grün: 558 Backend-Unit-, 277 Backend-Integrations- (davon 23 neu), 34 Migrationstests, 0 Schwachstellen ≥ high (Backend und Frontend), Migration Doctor `ready:true, applied:13`, 499 Frontend-Tests unverändert grün (keine Frontend-Änderung), Chromium-E2E-/Axe-Suite zweimal unabhängig 64/64. Vollständiger Bericht in `STAGE_5C1_ACCOUNT_DELETION_BACKEND.md`. Stage 2B2B bleibt weiterhin **Deferred until first customer / production deployment**; es wurde für Stage 5C1 keine Cloud-Infrastruktur eingerichtet, keine echten Benutzerdaten gelöscht (alle destruktiven Verifikationen liefen gegen disponible Testdatenbanken), und Stage 5C2 (Frontend-UI) wurde nicht begonnen.
>
> **Nachtrag (2026-07-28, Stage 5C1 Merge-Gate-Review):** Der oben beschriebene Stand war **noch nicht merge-bereit** — eine gezielte Prüfung fand fünf Befunde, alle vor erneuter Merge-Bereitschaft behoben: (1) ein echter Privat-zu-global-Leak bei persönlichen Übungen über `exercises.user_id`s `ON DELETE SET NULL` im Hard-Delete-Pfad; (2) ein unvollständiges Terminierungsregel-Scope (nur Ersteller, nicht auch das betroffene Mitglied); (3) persönliche Kalendereinträge wurden entgegen dem bereits korrekt gemergten Design unbedingt statt nur im `PLANNED`-Status gelöscht (Preview-Feld dafür von `futurePersonalCalendarEntries` zu `personalCalendarEntriesToDelete` umbenannt); (4) die No-CSRF-Entscheidung für den Löschendpunkt wurde als endgültige, verifizierte Architekturentscheidung bestätigt (ADR 004 entsprechend ergänzt); (5) der Deletion Receipt Doctor meldet jetzt bei jedem fehlenden Receipt sofort `ready:false`, nicht nur bei einem beschädigten. Volle Details in `STAGE_5C1_ACCOUNT_DELETION_BACKEND.md` Abschnitt 0 und ADR 004s Abschnitt „Amendment". 9 neue Integrationstests decken alle fünf Befunde ab (32 statt 23 in `accountDeletionApi.test.js`); vollständige Regression erneut grün (siehe Abschlussbericht für die exakten, nach dieser Korrektur gültigen Zahlen). Weiterhin: keine Cloud-Infrastruktur, keine echten Daten verändert, kein Stage-5C2-UI begonnen.
>
> **Nachtrag (2026-07-28, Receipt-first-Commit-Protokoll — Merge-Blocker-Fix):** Auch nach den fünf oben genannten Korrekturen war Stage 5C1 **noch nicht merge-bereit**: ein Receipt-Schreibfehler nach einem erfolgreichen Hard Delete führte zu einem vollständig unrekonstruierbaren Zustand — keine `users`-Zeile, kein Receipt, der Doctor meldete fälschlich `ready:true`. Live reproduziert und behoben durch ein **Receipt-first-Commit-Protokoll**: das Deletion Receipt wird jetzt innerhalb der noch offenen Transaktion aufgelöst (bestehendes gültiges Receipt wiederverwendet oder neu publiziert) und **erst danach** committet, statt wie zuvor als Best-Effort-Schritt nach dem Commit. Ein Publikationsfehler rollt die gesamte Transaktion zurück (kein HTTP 200); ein Commit-Fehler nach erfolgreicher Publikation lässt das Receipt bestehen und wird vom Doctor jetzt auch für Hard-Delete-Konten zuverlässig über denselben Mechanismus wie ein echter Restore erkannt, Reconciliation vervollständigt die Löschung idempotent unter Wiederverwendung desselben Receipts. Volle Details in `STAGE_5C1_ACCOUNT_DELETION_BACKEND.md` Abschnitt 0b. 11 neue, 1 gegenstandslos gewordener und entfernter Integrationstest (42 statt 32 in `accountDeletionApi.test.js`), 5 neue Unit-Tests; vollständige Regression erneut grün (563 Backend-Unit-, 296 Backend-Integrations-, 34 Migrationstests, 0 Schwachstellen ≥ high, Migration Doctor `ready:true, applied:13`, 499 Frontend-Tests unverändert grün, Chromium-E2E-/Axe-Suite zweimal unabhängig 64/64). Weiterhin: keine Cloud-Infrastruktur, keine echten Daten verändert, kein Stage-5C2-UI begonnen.

> **Nachtrag (2026-08-09, Stage 5C2 Account Deletion UI / Profile Danger Zone):** Die in Stage 5C1 offen gelassene Lücke "keine Frontend-Oberfläche" ist jetzt geschlossen. Neue Komponente `AccountDeletionDangerZone.vue`, eingehängt im Profil-Tab "Sicherheit": ein Preview-Dialog zeigt ausschliesslich die tatsächlichen Backend-Auswirkungen (gruppiert in "Wird entfernt"/"Kann erhalten bleiben"/"Hinweise", Zero-Counts als "Keine." zusammengefasst statt einzeln aufgelistet), ein Sole-Owner-Blocker zeigt betroffene Studio-Namen sachlich ohne Execute-Pfad, und ein zweiter expliziter Bestätigungsschritt (aktuelles Passwort + exakte Bestätigungsphrase = eigener Benutzername, wie vom Stage-5C1-Vertrag tatsächlich verlangt) schliesst mit vollständigem Auth-/Studio-Context-Cleanup und Redirect zu `/login` ab — kein neuer Logout-Request, da die Löschtransaktion jede Session bereits serverseitig widerruft. Keine neue Backend-Fachlogik. **Wichtigster Fund dieser Phase:** beim Bau des E2E-Flows wurde ein echter, vorbestehender Stage-5C1-Backend-Defekt entdeckt — `accountRouter.js` registriert den `account.deleteRequest`-Rate-Limiter **vor** `authenticate`, wodurch dessen Schlüssel (`req.user?.id`) immer `undefined` ist und **alle** Aufrufer einen einzigen geteilten Bucket teilen, statt wie dokumentiert pro Benutzer isoliert zu sein (dasselbe Muster betrifft auch `change-password`/`email-change-requests`). Bewusst **nicht** in dieser reinen Frontend-Phase behoben (siehe `STAGE_5C2_ACCOUNT_DELETION_UI.md` Abschnitt 13 und `FITTRACK_SECURITY_AND_PRIVACY_STATUS.md`), stattdessen mit einem minimalen, reproduzierbaren Regressionstest dokumentiert. Vollständig grün (frischer `npm ci` in beiden Workspaces): 564 Backend-Unit- (563 + 1 neuer Defekt-Nachweistest), 296 Backend-Integrations- (unverändert), 34 Migrationstests, Migration Doctor `ready:true, applied:13` (gegen eine eigens erzeugte und wieder abgebaute Scratch-Datenbank, die persistente lokale Dev-DB wurde nicht verändert), 0 Schwachstellen ≥ high (Backend und Frontend), 531 Frontend-Tests (502 + 29 neue), Produktionsbuild erfolgreich, Chromium-E2E-/Axe-Suite zweimal unabhängig 67/67 (64 + 3 neue), 0 fehlgeschlagen, 0 übersprungen, keine unerwarteten Retries, keine kritischen/schweren Axe-Befunde. Vollständiger Bericht in `STAGE_5C2_ACCOUNT_DELETION_UI.md`. Stage 2B2B bleibt weiterhin **Deferred until first customer / production deployment**; keine Cloud-Infrastruktur, keine echten Benutzerkonten gelöscht (alle destruktiven Verifikationen liefen gegen disponible Test-/E2E-Datenbanken), keine Migration 014, keine Admin-Löschung fremder Konten, keine Studio-Membership-Removal-UI.

> **Nachtrag (2026-08-09, PR #29 — authenticated-per-user Rate-Limiter-Reihenfolge behoben):** Der oben beschriebene Rate-Limiter-Defekt ist jetzt behoben — `authenticate` läuft in `accountRouter.js` seit diesem Hotfix vor `rateLimiters.deleteRequest`/`passwordChange`/`emailChangeRequest`, bewiesen durch neue Route-Level-Integrationstests (`backend/test/integration/accountRateLimitIsolation.test.js`), die vor dem Fix den Defekt reproduzierten und danach grün waren. Der auf Stage 5C2 hinzugefügte, jetzt gegenstandslose `KNOWN DEFECT`-Unit-Test wurde entfernt (nicht auf eine falsche Erwartung "korrigiert"), da die stärkere Route-Level-Abdeckung ihn vollständig ersetzt. Stage 5C2 wurde danach mit `main` synchronisiert. Details: `docs/STAGE_5D_CURRENT_STATE_AUDIT.md`.
>
> **Nachtrag (2026-08-19, Stage 5D Current-State Audit):** Vollständiges Audit nach Stage 5C2 durchgeführt, keine neue Produktfunktion. Alle Testzahlen frisch gemessen und bestätigt: 251 Backend-Syntax-Dateien, 563 Backend-Unit-, 305 Backend-Integrations-, 34 Migrationstests, Migration Doctor `ready:true, applied:13`, 0 Backend-Schwachstellen ≥ high, 531 Frontend-Tests, erfolgreicher Produktionsbuild, Chromium-E2E-/Axe-Suite zweimal ausgeführt. **Neuer Fund:** ein High-Severity-`npm audit`-Advisory im Frontend (`nanoid@3.3.17`, GHSA-2v37-7h3g-55p8, transitiv über `postcss`←`vite`/`@vue/compiler-sfc`, keine Backend-Betroffenheit) — nicht behoben (Dependency-Upgrades ausserhalb des Scopes dieses reinen Doku-Audits), siehe `STAGE_5D_CURRENT_STATE_AUDIT.md` Abschnitt 19 für Empfehlung. Ausserdem im Katalog nachgetragen: `FITTRACK_API_CATALOG.md` fehlten die gesamte Training-Calendar- (Stage 5A1) und Assignment-Schedule-Rules-Gruppe (Stage 5A3) vollständig, und die "Kein Frontend-Nutzer"-Zeilen für Workout-Sessions/Coach-Resultate waren seit Stage 1B.2B2A/1B.2B2B veraltet — beide jetzt korrigiert. Volle Details, Funktionsinventar und Pilot-/Produktions-Readiness-Neubewertung: `docs/STAGE_5D_CURRENT_STATE_AUDIT.md`. Stage 2B2B bleibt weiterhin **Deferred until first customer / production deployment**; keine Cloud-Infrastruktur eingerichtet, keine Produktcodeänderung, keine Migration, keine Dependency-Änderung in diesem Audit.
...
Siehe `FITTRACK_SECURITY_AND_PRIVACY_STATUS.md`, Abschnitt "Datenschutzklassifikation". Die Satzresultate (`studio_workout_session_sets`) sind als sensibelste Datenklasse (P4) identifiziert, mit der striktesten Zugriffsregel im System (kein Owner-/Admin-Bypass) und explizit nachgewiesener Abwesenheit in Logs/Audit. ~~Es existiert kein Recht-auf-Löschung-/Anonymisierungsmechanismus für Benutzerdaten irgendwo im Code~~ — **[KORRIGIERT, Stage 5C1/5C2]** Zum Stand von PR #7 (2026-07-19, geprüfter Commit dieses Abschnitts) zutreffend, seit Stage 5C1 (Backend, `POST /api/account/deletion-request`) und Stage 5C2 (Profil-Oberfläche) **nicht mehr aktuell**: Anonymisierung oder Hard Delete des eigenen Kontos ist vollständig implementiert und automatisiert getestet, siehe `docs/STAGE_5D_CURRENT_STATE_AUDIT.md` Abschnitt 11/12.
