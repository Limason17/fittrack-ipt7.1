...
> **Nachtrag (2026-07-25):** Die ursprünglich hier beschriebene `localStorage`-Write-dann-Readback-Mutex-Implementierung erwies sich als **nicht ausreichend exklusiv** — die damalige Einschätzung „ein verbleibendes Restrisiko ... ist nicht schlechter als der Zustand ganz ohne Koordination" wurde durch einen tatsächlich reproduzierbaren Cross-Tab-Race widerlegt: zwei Tabs konnten beide „no lock" lesen, bevor eines von beiden geschrieben hatte, und beide hielten sich anschließend (je über ihren eigenen Readback) für den Owner. Das führte zu echten doppelten `/auth/refresh`-Aufrufen mit demselben, noch nicht rotierten Cookie und damit zu einer **fälschlichen** `AUTH_REFRESH_REUSE_DETECTED`/`AUTH_SESSION_INVALIDATED`-Kaskade für einen vollständig legitimen Ablauf. Der Mechanismus wurde durch eine tatsächlich exklusive `navigator.locks`-Primitive als primären Weg ersetzt, mit einem gehärteten (Owner-ID + Lease-Ablauf + Generation/Fencing + Re-Verifikation unmittelbar vor dem Netzwerkrequest + Jitter-Retry statt reinem Read-then-Write) Fallback für Umgebungen ohne Web-Locks-Unterstützung. Vollständige Root-Cause-Analyse, der neue Vertrag und der Stabilitätsnachweis stehen in `STAGE_3C_PILOT_UX_POLISH.md`, Abschnitt „Cross-Tab-Refresh-Race-Härtung". Die folgenden Punkte sind der **historische** Stand zum Zeitpunkt von Stage 3B2 und bewusst nicht rückwirkend umgeschrieben.
...
- ~~Bewusst kein formal atomarer verteilter Lock ... ein verbleibendes Restrisiko exakt gleichzeitiger Schreibvorgänge ist nicht schlechter als der Zustand ganz ohne Koordination.~~ **Widerlegt, siehe Nachtrag oben.**

Verifiziert sowohl unit- (`refreshCoordination.test.js`: Lock-Erwerb, Warten, Freigabe, Stale-Lock-Verfall, keine Tokenwerte im Lock) als auch E2E-seitig (`authSession.spec.js`: "two tabs of the same browser context refreshing at nearly the same moment never treat a legitimate user as token theft" — zwei echte Tabs, echter gleichzeitiger Reload via `Promise.all`, alle `/auth/refresh`-Aufrufe erfolgreich, kein Tab landet auf der Login-Seite). **Diese Verifikation stammt aus der Stage-3B2-Sitzung selbst und deckte den in obigem Nachtrag beschriebenen Race nicht auf** — er trat erst bei wiederholten (`--repeat-each`) Läufen zuverlässig zutage; siehe `STAGE_3C_PILOT_UX_POLISH.md` für den aktuellen, mit 20 Wiederholungen plus zwei vollständigen Suite-Läufen verifizierten Stand.

**Verbleibende Grenze**: die Koordination wirkt nur innerhalb *eines* Browser-Kontexts (echte Tabs desselben Fensters/derselben Profil-Instanz). Zwei völlig unabhängige Browser-Kontexte oder Geräte (z. B. Desktop + Handy) haben systembedingt getrennte Cookie-Jars und damit unabhängige Sitzungen — das ist beabsichtigtes, korrektes Verhalten (Abschnitt 12), kein Koordinationsfall. Diese Grenze gilt unverändert auch für den neuen `navigator.locks`-basierten Mechanismus.
